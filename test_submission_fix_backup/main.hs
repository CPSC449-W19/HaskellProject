...
--fpaOkay :: String -> Int
--fpaOkay x if (length x == 5) and (x !! 0 == '(') and (validMach (x !! 1) == True) and (x !! 2 == ',') and (validTask (x!!3) == True) and (x !! 4 == ')') then 0
...
main = do
  ...
  if last errList1 == "Error while parsing input file" then hPutStrLn theFile $ last errList1 else if
...
{-
  if last errList1 == "Error while parsing input file" then writeFile argsOut (show $ last errList1) else if
     ((last $ init errList1) /= "forbidden machine:") then writeFile argsOut (show $ last $ init errList1) else if
     ((last $ init $ init errList1) /= "too-near tasks:") then writeFile argsOut (show $ last $ init $ init errList1) else if
     ((last $ init $ init $ init errList1) /= "machine penalties:") then writeFile argsOut (show $ last $ init $ init $ init errList1) else if
     ((last $ init $ init $ init $ init errList1) /= "too-near penalities") then writeFile argsOut (show $last $ init $ init $ init $ init errList1) else if
     ((last $ init $ init $ init $ init $ init errList1) /= "cool") then writeFile argsOut (show $last $ init $ init $ init $ init $ init errList1) else writeFile argsOut (show out)
  --hClose theFile
-}
...
{-
  let err = ["first"]
  let out = getOutput fpa fm tnt mp tnp err
  theFile <- openFile argsOut WriteMode
